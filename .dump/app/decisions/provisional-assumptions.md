...
- PA-8. Native refuses plan mode, offline/Ollama, and custom endpoints with a
  visible error instead of silent fallback, because silent fallback would run
  turns under different credentials/safety than the user selected. Overturn any
  refusal into support only when the capability genuinely exists.
