...
A second batch was answered 2026-09-14 and is recorded in the same file, `.dump/global/decisions.md`. Five of its answers move steps: `sharp` is declared as a devDependency by step 12 rather than retiring the icon script, package identity is closed at `dev.mausinc.mauscode` with the display name `mausCode` and a CI branding guard in step 31, the data-egress doctrine is the acceptance test for step 27 and now a rule in `AGENTS.md`, the vendored contracts are kept and absorbed per use with the ledger at `.dump/app/plans/contracts-adoption.md`, and step 32 defines alpha and stable channels; signing was then refused outright in batch 4 the same day, so that step ships unsigned by design with all four targets still in its matrix. The sixth closes handoff-document hygiene as a convention in `AGENTS.md`, with no root `HANDOFF.md` tracked.

## 4b. Known drift between the issues and these files

GitHub issue bodies were written from these files, and the integration this session runs under cannot edit an issue, comment on one, add a label to one, or, as of 2026-09-14, even create one; every one of those calls returns 403 `Resource not accessible by integration`, although issue creation succeeded when #3 through #48 were filed an hour earlier. Reads succeed and repo-level label writes still work. The first item below was cleared by the human on 2026-09-14: all 46 issues carry the `roadmap` label and the probe issue is deleted, verified with `gh issue list --label roadmap --limit 200` in this session. The rest was then reviewed and accepted, so this section is a description of the intended state, not a queue of fixes. Ratified: the files are the source of truth, no re-sync is scheduled, and widening the GitHub scope is not required to work the program.

- Steps 02, 03, 04, 05, 12, 17, 24, 27, 31, 32, 43 and 45 have a body behind their file, because the ratifications above landed after the bodies were written. `NN-<slug>.md` is the truth, the issue is a pointer. Thirteen dated drift notices, one per lagging step plus the index, are written and dry-run at `.dump/app/plans/2026-09-14-issue-drift-notices.md` with a posting loop, and they remain **unposted** because this integration cannot comment; a human runs the loop.
- The drift stopped being cosmetic on 2026-09-14: an automatic reviewer has already generated implementation plans from these bodies, and two of them (#7 and #14) encode decisions the human reversed, while #34's plan reports a checkout with no `.dump/` tree at all. See §0a of that file before trusting any bot-authored plan for these steps.
- **Thirty-eight bodies resolve cross-references as `{{SNN}}` tokens instead of issue numbers, not six.** Measured 2026-09-14 by reading all 46 `roadmap` bodies: 314 tokens in total, 239 distinct, of which 97 sit outside the `Depends on` and `Blocks` rows, where a token was meant to be a number. That is 38 of the 46 bodies. The eight that carry none outside those rows are #3, #30, #31, #32, #33, #34, #36 and #44. The cause is unchanged, `PATCH` was unavailable when the numbers were back-filled, and the reading rule is unchanged: `{{SNN}}` is issue `#NN+2`.
- The `roadmap` label is applied to all 46 issues, #3 through #48, verified 2026-09-14 by the human in the web UI after the integration proved it could create issues but not label one. Step 22's issue adapter is therefore unblocked, and its trigger label exists in the repository. An agent that finds an unlabelled issue in this range applies the label rather than assuming the range is unmanaged.
...
## 4c. State at handoff, 2026-09-14

Planning is the deliverable of the session that produced this file. The human declined, on 2026-09-14, to execute any step from it: no dependency reinstall in the sandbox, no step 01 commit, and no parallel sub-chat fan-out on the first wave. Steps 01, 02 and 03 are ready to be taken in that order by whoever is assigned, each carrying its own verification obligation and its `.dump` benchmark record, and nothing in the second half of the table may start before its dependency column is merged.
