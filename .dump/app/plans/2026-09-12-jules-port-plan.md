...
| "app build, then packaging" | not attempted here (needs `bun install`; deliberately not run in this sandbox) | run on a machine with deps: `bun install --frozen-lockfile && bun run build:runtime-client && bun run build`, then `package:mac`; gates `typecheck`/`lint`/`test` before packaging |
