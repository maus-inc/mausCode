...
| P1-5 | One Codex model catalog; `gpt-5.5` vs the note's `gpt-5.4` | **Closed 2026-09-15, step 05 shipped.** Both constants are deleted and `grep -rn "DEFAULT_CODEX_MODEL\b" src` returns nothing. `src/main/lib/providers/codex-models.ts` reads the catalog from the pinned CLI over the app-server `model/list` request, takes the entry flagged `isDefault` with its `defaultReasoningEffort`, validates the id against the static picker list before using it, caches against `codex --version`, and falls back to `src/shared/codex-model-id.ts` with the reason surfaced in `fallbackReason`. A turn calls `peekCodexDefaultModel`, which never spawns. Follow-up, not done here: nothing in the renderer calls `codex.getDefaultModel`, so the picker still shows its own static default and cannot show that the CLI answer fell back. See §5.3. |
