...
4. **ACP (Agent Client Protocol)** — for CLIs that speak it (qwen, cline,
   ...
   `@mcpc-tech/acp-ai-provider` path already used by cursor/gemini routers.
...
- Repo already drives cursor via ACP; the native print/stream-json path is
  the documented upgrade route (fuller fidelity, resume, partial streaming).
  Source: [headless](https://cursor.com/docs/cli/headless).

### 5. grok build (xAI)

- Official **Grok Build CLI**: `curl -fsSL https://x.ai/cli/install.sh |
  bash` (macOS/Linux), `irm https://x.ai/cli/install.ps1 | iex` (Windows).
  Terminal-native, codebase-aware, plan-before-code; MCP support reported
  limited vs Claude Code; billing = xAI API credits.
- Community fallback: `superagent-ai/grok-cli` (MIT, Bun/OpenTUI):
  `--prompt` headless mode, MCP, subagents on by default, Telegram remote,
  `.grok/GROK.md` project instructions, `GROK_BASE_URL` override.
- Models: grok-code-fast-1, grok-4-latest (256k ctx), grok-3 family; xAI API
  is OpenAI-compatible (raw-API fallback rung).
