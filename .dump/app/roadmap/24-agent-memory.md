...
2b. Ownership is hybrid by ratification, the runtime may propose and never commit. One `proposals` queue for every agent-initiated write, memory entries here and skill proposals from step 43, each row carrying provenance, and one accept surface in the details sidebar. The engine's own memory store stays dark, because two writers for one fact is how it ends up stored twice with different content.
