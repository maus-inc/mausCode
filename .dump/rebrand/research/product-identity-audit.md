...
| `build.appId` | `dev.21st.agents` | TECHNICAL (reverse-domain app id) | → `dev.mausinc.mauscode`. Fresh product: no existing installs to migrate, so the id changes. `dev.` prefix follows the inherited convention; domain is a placeholder pending the control-plane domain decision (see open-decisions.md D4) |
...
| `index.ts` menu | `About 1Code`; `Install '1code' Command...` + dialogs (×4) | PRODUCT BRANDING + command name | → `About mausCode`; `'mauscode'` (pending user confirmation — open-decisions.md D1) |
