...
## Real candidates (delta vs upstream tip)`sierra
...
- Sub-chat: `stores/sub-chat-store.ts`, `ui/sub-chat-selector.tsx`, `ui/sub-chat-status-card.tsx`, `main/active-chat.tsx` — TAKE carefully (sidebar-lineage overlap with sylv — resolve ONE lineage before merging; our `agents-sidebar` untouched пока)
