...
- 2026-09-11(j): USER DECISIONS: Cursor provider APPROVED (Phase 6b queued); sidebar lineage DECIDED = adopt erenbertr (Batch C started).
- 2026-09-11(i): Phase 6 REVIEWED + micro-ported (1code-ui): cli-binaries/PATH fallback, subscription-only filter, hidden v5. Cursor adoption HELD for user (brief in .dump/app/decisions); web-standalone + vendor-auth-optional rejected. tsc identical-31, 70/70, clean.
- 2026-09-11(o): session-binding IMPLEMENTED our way (Locus take #1): provider TEXT on sub_chats + 0010 migration, shared union, chats router seed/copy/update, renderer binding-first read + write-through + lazy backfill. tsc zero-delta (25), 27/27, secrets clean, migration sqlite-verified.
- 2026-09-11(n): T3 t3code server REVIEWED + HELD (MIT): serve/pairing/DPoP/MCP-HTTP/DesktopServerExposure all built on Effect (effect/*, effect/unstable/http+ai) + @t3tools contracts/shared/tailscale workspace pkgs. No shared substrate with our Electron+tRPC+Drizzle tree; any adoption = replatform decision, not harvest. No file-level takes. Needs explicit user/product approval (stays Phase 5, not committed).
- 2026-09-11(m): Locus SCOPED (115 archived changes; Apache-2.0; zh docs): no blind ports. Ranked queue: (1) chat-session-binding core idea — implement OUR way via own OpenSpec (DB binding truth kills infer-fragility, worse now at 5 providers; their 595+210-line owner module rides their Run/job arch, not portable); (2) provider-diagnostics taxonomy concepts; (3) local-only guard (needs product decision). Reject/queue rest: capability model, workbench, jobs, headless, i18n, registry, mcp-import-preview (no consumer even in their tree), chat-base-commit/file-stats (their-UI-specific).
- 2026-09-11(l): ning REJECTED wholesale (Apache-2.0, no CJK in src/): security-mining is zh-only pentest niche (Findings.md + report.md, VoidForge refs); tooling/ 6.7k-line subsystem is an arch fork (private ~/.1code/.claude home, ProviderAdapter, thin-router rewrites) needing product decision + conflicts our MCP+cursorsystem; skills-router/tab exts require that backend; skill-md frontmatter + voice already-have (voice byte-identical).
- 2026-09-11(k): sylvain MINE (dev-server only): detect router + hook + button + constants + running-atom + active-chat render; FR->EN, tabs->spaces, @/->relative. REJECT rest: archive/reorder already-have, emoji (dep+pipeline weight, cosmetic), 5-mode taxonomy HELD (user-facing). tsc zero-delta (25), 27/27, secrets clean.
- 2026-09-11(k2): jhckevin REJECTED wholesale (reverses earlier standalone TAKE): backend-route has no consumer outside its app-server arch; banner-model/changelog-url logic already inline in ours; release-config overlaps generate-update-manifest + branded env, no consumer.
- 2026-09-11(j): Phase 6b Cursor CLI provider ADOPTED (1code-ui 51b79a5+12f0676): router/binary/mcp/model-id/transport/login-modal-content-hook new; selector+form+input+infer+MCP-tab wiring; main stream guards. tsc zero-delta (25), node--test 27/27, secrets clean. Open: Composer model-ID re-validation vs live cursor-agent.
- 2026-09-11(i): Phase 3 Batch C DONE (unified-sidebar set): rail+all-projects port, shell trio (folder/vscode/terminal), sidebar install w/ Kanban keep, content rail routing + AnimatePresence, subchats-pane deletion, Cmd+T rebind (Opt+Cmd+T new sub-chat; +Shift = sub-chat tab), usage-widget retire. tsc 31->25 zero-new (6 fixed), node--test 27/27, secrets clean.
- 2026-09-11(h): Phase 3 Batch B DONE (remainder triage, 5-file take): mention plugin-source, mem-trace completion; all other diffs verified deliberate keeps/rejects. tsc 33->31 zero-new, 70/70 tests, secrets clean. Renderer harvest COMPLETE except held sidebar set.
