...
- Typecheck is a zero-error gate. `npm run typecheck` runs `tsc --noEmit`, and `.github/ci-baselines/typecheck.txt` is empty, which means the ratchet allows no errors at all. `npm run ts:check` runs `tsgo`, which is not wired into CI yet; roadmap step 02 lands it as a second gate after measuring and justifying every disagreement with `tsc`, so until that step merges `tsgo` output is advisory and `tsc` is the gate.
