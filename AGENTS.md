...
- The runtime event mapper at `src/main/lib/runtime/translate.ts` keeps 19 harness events chunk-internal on purpose and records `compacted`, `session_status`, `background_progress` and `wake_requested` as run events; an unmapped kind logs once. A feature that needs one of the internal ones extends that switch and its test, and does not fake the state in the renderer. The per-event table lives in `.dump/app/research/2026-09-13-event-mapping.md`.
...
UI and design work loads a fixed set ahead of any code, every time. Open `DESIGN.md`, then `docs/design-system-baseline.md`, then `antislop` plus the antislop skill that matches the task, then `impeccable` with the `reference/` playbook its Commands table routes to, then the taste set led by `design-taste-frontend` with the style skill the surface routes to, then `ui-ux-pro-max`, then `unslop` for every string a user reads. The taste set from `Leonxlnx/taste-skill` is mandatory for any UI or design work, a landing page or a marketing screen, a portfolio, a redesign, or a component and motion detail on an existing surface, and its pre-flight check runs before the work is called done. `docs/design-skills.md` carries the load order, the routing table for the rest of the set, and what each skill may not execute in this repository, which includes impeccable's launcher, the remote image generators in `design` and in the taste image skills, and every `npm install` a skill asks for. A skill's own setup step never outranks that file.
...
| design-set | any UI, UX, motion, typography or copy work | Load `DESIGN.md`, `docs/design-system-baseline.md`, the taste set led by `design-taste-frontend`, and the rest of the skill set in `docs/design-skills.md` before you write code, run the taste pre-flight, then deliver through its six gates, starting with the antislop Delivery Gate. |
