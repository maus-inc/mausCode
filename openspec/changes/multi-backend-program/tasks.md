...
- [x] 8. cursor native (print/stream-json/resume upgrade from ACP path: args builder + NDJSON parser + mock tests 18/18, retry loop, status probe, cursor-agent-first binary resolution, project+global mcp.json, ACP dep removed from cursor path; research batch 1+2 in session record)
