...
import { JcodeClient, HarnessError, NdjsonDecoder, unixSocketTransport } from "../dist/index.js"
...
test("run() collects a full turn and auto-approves permissions", async () => {
  ...
  assert.deepEqual(turn.toolCalls, [{ callId: "c1", name: "bash", output: "ok", error: undefined }])
...
test("softInterrupt preserves images and the legacy text-only wire shape", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      received.map(({ v, id, ...request }) => request),
      [
        {
          req: "soft_interrupt",
          session_id: "s1",
