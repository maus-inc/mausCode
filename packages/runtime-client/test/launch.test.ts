...
test("a missing jcode binary is catchable, and leaks nothing", async () => {
  ...
  const after = fs.readdirSync(root).filter((name) => name.startsWith("jcode-sdk-instance-")).length
  assert.equal(after, before, "a failed launch must not leave an instance home behind")
})

test("swarmModel reaches the launched runtime and overrides the generic environment", {
...
test("wakeMode reaches the launched runtime and overrides the generic environment", {
  skip: process.platform === "win32",
}, async () => {
  const { launchInstance } = await import("../dist/launch.js")
