...
import { type LaunchedInstance, type LaunchOptions, launchInstance } from "./launch.js"
import {
  API_VERSION_MAJOR,
  type ApiEvent,
  type ApiRequest,
...
export function unixSocketTransport(socketPath: string): Promise<Transport> {
  return new Promise((resolve, reject) => {
    ...
    const onConnectError = (cause: NodeJS.ErrnoException) => reject(connectError(socketPath, cause))
...
export class JcodeClient extends EventEmitter {
  ...
  static async connect(options: ConnectOptions = {}): Promise<JcodeClient> {
    ...
    // socketPath is always defined when no custom transport is set; the fallback only satisfies the type.
    const transport =
      options.transport ?? (await unixSocketTransport(socketPath ?? apiSocketPath()))
      ...
      if (this.requestTimeoutMs > 0) {
        entry.timer = setTimeout(() => {
          ...
          reject(
            new HarnessError(
              "timeout",
              `no reply to ${request.req} within ${this.requestTimeoutMs}ms`,
            ),
          )
  ...
  async getHistoryWithImages(
    ...
    const frame = await this.expectReply({ req: "get_history", session_id: sessionId }, "history")
  ...
  globalEvents(options: GlobalEventsOptions = {}): AsyncIterableIterator<ApiEvent> {
    ...
    const startChild = async (sessionId: string): Promise<void> => {
      ...
      try {
        ...
        await child?.close()
        if (!stopped && !(error instanceof HarnessError && error.code === "unknown_session")) {
      ...
      next: () => {
        ...
        if (waiter) {
          return Promise.reject(
            new HarnessError(
              "concurrent_next",
              "globalEvents does not support concurrent next() calls",
            ),
          )
        }
