/**
 * mausCode GrokChatTransport: ChatTransport<UIMessage> over trpc.grok.chat.
...
import { DEFAULT_GROK_UI_MODEL } from "../../../../shared/grok-model-id"
import { grokLoginModalOpenAtom, sessionInfoAtom } from "../../../lib/atoms"
import { appStore } from "../../../lib/jotai-store"
import { trpcClient } from "../../../lib/trpc"
import { pendingAuthRetryMessageAtom, subChatGrokModelIdAtomFamily } from "../atoms"
...
function getSelectedGrokModel(subChatId: string): string {
  const selectedModelId = appStore.get(subChatGrokModelIdAtomFamily(subChatId))
  const selectedModel = GROK_MODELS.find((model) => model.id === selectedModelId) || GROK_MODELS[0]

  return selectedModel?.id || DEFAULT_GROK_UI_MODEL
}

export class GrokChatTransport implements ChatTransport<UIMessage> {
  constructor(private config: GrokChatTransportConfig) {}
  ...
  async sendMessages(options: {
    ...
    const selectedModel = getSelectedGrokModel(this.config.subChatId)
    ...
    return new ReadableStream({
      start: (controller) => {
        ...
        sub = trpcClient.grok.chat.subscribe(
          {
            ...
            onData: (chunk: UIMessageChunk) => {
              ...
              if (chunk.type === "auth-error") {
                ...
                void (async () => {
                  ...
                  try {
                    const integration = await trpcClient.grok.getIntegration.query()
                  ...
                  appStore.set(grokLoginModalOpenAtom, true)
                  if (isConnected) {
                    toast.error("Grok authentication failed", {
                      description:
                ...
                void trpcClient.grok.cleanup
                  .mutate({ subChatId: this.config.subChatId })
                  .catch(() => {
                    // No-op
                  })

                controller.error(new Error("Grok authentication required"))
                return
              }

              if (chunk.type === "error") {
                toast.error("Grok error", {
                  description: chunk.errorText || "An unexpected Grok error occurred.",
                })
              }
        ...
        options.abortSignal?.addEventListener("abort", () => {
          const cancelPromise = trpcClient.grok.cancel
  ...
  cleanup(): void {
    void trpcClient.grok.cleanup.mutate({ subChatId: this.config.subChatId }).catch(() => {
      // No-op
    })
  }
