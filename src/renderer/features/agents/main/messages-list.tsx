...
function createMessageStore(): MessageStore {
  ...
  function getMessageSnapshot(msg: Message) {
    ...
    return {
      ...
      textLengths: parts.map((p: { type?: string; text?: string }) =>
        p.type === "text" ? p.text?.length || 0 : -1,
      ),
      partStates: parts.map((p) => ("state" in p ? p.state : undefined)),
    }
  }
...
export function useMessage(messageId: string) {
  ...
  const prevMessageRef = useRef<Message | null | undefined>(null)
...
export const SimpleIsolatedGroup = memo(function SimpleIsolatedGroup({
  ...
  const imageParts =
    userMsg?.parts?.filter(
      (p): p is { type: `data-${string}`; id?: string; data: ImagePartData } =>
        p.type === "data-image",
    ) || []
