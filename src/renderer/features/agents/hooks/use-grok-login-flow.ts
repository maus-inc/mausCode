/**
 * mausCode useGrokLoginFlow: device-auth session polling + verify.
...
export type GrokLoginFlowState = "idle" | "running" | "success" | "error" | "cancelled"

const VERIFY_ATTEMPTS = 6
const VERIFY_DELAY_MS = 400

function isTerminalState(state: GrokLoginFlowState): boolean {
...
export function useGrokLoginFlow() {
  const [sessionId, setSessionId] = useState<string | null>(null)
  const [state, setState] = useState<GrokLoginFlowState>("idle")
  ...
  const startLoginMutation = trpc.grok.startLogin.useMutation()
  const cancelLoginMutation = trpc.grok.cancelLogin.useMutation()
  const openExternalMutation = trpc.external.openExternal.useMutation()
  const trpcUtils = trpc.useUtils()

  const sessionQuery = trpc.grok.getLoginSession.useQuery(
    { sessionId: sessionId || "" },
    {
  ...
  const verifyConnectedSession = useCallback(
    async (verificationKey: string) => {
      ...
      for (let attempt = 0; attempt < VERIFY_ATTEMPTS; attempt += 1) {
        try {
          ...
          if (integration.isConnected) {
            await trpcUtils.grok.getIntegration.invalidate()
            setState("success")
            setError(null)
            if (successToastSessionRef.current !== verificationKey) {
              successToastSessionRef.current = verificationKey
              toast.success("Grok connected successfully", { duration: 10000 })
            }
            return
          }
        } catch (verifyError) {
      ...
      const message = lastVerifyError
        ? toErrorMessage(lastVerifyError, "Failed to verify Grok login status. Please retry.")
        : "Grok login completed, but credentials were not detected. Please retry."
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      toast.info("Waiting for Grok sign-in in your browser")
    } catch (startError) {
      if (wasCancelled()) {
        return
      }

      const message = toErrorMessage(startError, "Failed to start Grok login. Please try again.")
      setState("error")
      setError(message)
      notifyError(message)
    } finally {
  ...
  useEffect(() => {
    ...
    if (data.state === "success") {
      const verificationKey = data.sessionId || sessionId || "grok-login"
    ...
    if (data.state === "error") {
      const message = data.error || "Grok login failed. Please retry."
      if (message) {
        notifyError(message)
      }
    }
  ...
  useEffect(() => {
    return () => {
      ...
      void trpcClient.grok.cancelLogin.mutate({ sessionId }).catch(() => {
        // No-op
      })
    }
  }, [sessionId, state])
