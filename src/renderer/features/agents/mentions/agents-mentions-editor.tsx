...
function buildContentFromSerialized(
  ...
  const regex = /@\[([^\]]+)\]/g
  let lastIndex = 0
  let match: RegExpExecArray | null = regex.exec(serialized)
...
export const AgentsMentionsEditor = memo(
  forwardRef<AgentsMentionsEditorHandle, AgentsMentionsEditorProps>(function AgentsMentionsEditor(
    ...
    const restoreCursor = useCallback((offset: number) => {
      ...
      let node: Node | null = walker.nextNode()
      let _lastTextNode: Text | null = null
      let _lastTextNodeOffset = 0
      ...
      while (node) {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          _lastTextNode = textNode
          _lastTextNodeOffset = nodeLength
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: mount-only editor init; re-running would clobber editing state.
    useEffect(() => {
      ...
      () => ({
        ...
        clearSlashCommand: () => {
          ...
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            while (walkNode) {
              if (walkNode === node) {
                localSlashPosition = (slashTriggerStartIndex.current ?? 0) - serializedCharCount
                break
              }
    ...
    return (
      <div className="relative">
        ...
        {/* biome-ignore lint/a11y/noStaticElementInteractions: contentEditable editor is natively keyboard-editable. */}
        <div
          ref={editorRef}
