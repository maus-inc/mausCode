...
import { getErrorMessage, useFileContent } from "../hooks/use-file-content"
...
function LoadingSpinner() {
  return <AppLoader label="Loading file..." layout="flex" />
}
  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault()
            handleEditorAction("editor.action.revealDefinition")
          }
        }}
      >
        Go to Definition
      ...
      <div
        ...
        tabIndex={0}
        className={itemClass}
        onClick={() => handleEditorAction("editor.action.goToReferences")}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault()
            handleEditorAction("editor.action.goToReferences")
          }
        }}
      >
        Go to References
      ...
      <div
        ...
        tabIndex={0}
        className={itemClass}
        onClick={() => handleEditorAction("editor.action.goToSymbol")}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault()
            handleEditorAction("editor.action.goToSymbol")
          }
        }}
      >
        Go to Symbol...
      ...
      <div
        ...
        tabIndex={0}
        className={itemClass}
        onClick={() => handleEditorAction("editor.action.quickCommand")}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault()
            handleEditorAction("editor.action.quickCommand")
          }
        }}
      >
        Command Palette
...
function CodeViewer({
    ...
    (monacoEditor: editor.IStandaloneCodeEditor, monacoInstance: Monaco) => {
      ...
      if (editorContainer) {
        const obs = new MutationObserver(() => {
          ...
          if (findWidget) {
            findWidget.querySelectorAll("[title]").forEach((el) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col h-full bg-background">
        <CodeViewerHeader fileName={fileName} filePath={filePath} onClose={onClose} />
        <LoadingSpinner />
      </div>
    )
  }
