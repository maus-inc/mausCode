...
export function useDevServer({ chatId, scopeKey, cwd }: UseDevServerArgs) {
  ...
  const [_allActiveIds, setAllActiveIds] = useAtom(activeTerminalIdAtom)
  ...
  useEffect(() => {
    ...
    paneId,
    scopeKey, // Backend doesn't have it (or it's dead) — purge any ghost
    setIsRunning,
    trpcUtils.terminal.getSession.fetch,
    setAllTerminals,
    setAllActiveIds,
    allTerminals,
  ])
