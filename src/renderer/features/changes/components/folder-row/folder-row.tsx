...
export function FolderRow({
  ...
  return (
    <CollapsibleRow
      ...
      triggerClassName={cn("text-xs items-stretch py-0.5", isGrouped && "text-muted-foreground")}
      contentClassName={cn(isGrouped && "ml-1.5 border-l border-border pl-0.5")}
      header={
        <FolderRowHeader name={name} level={level} fileCount={fileCount} isGrouped={isGrouped} />
      }
    >
      {children}
    </CollapsibleRow>
