...
export const HistoryView = memo(function HistoryView({
  ...
  const { data: commitFiles, refetch: refetchFiles } = trpc.changes.getCommitFiles.useQuery(
  ...
  const _handleFileClick = useCallback(
    (file: ChangedFile) => {
      if (selectedCommitHash) {
...
const _CommitFileItem = memo(function CommitFileItem({
  file,
  isSelected,
  onClick,
}: {
