/**
 * mausCode ClineLoginModal shell (auth-retry aware). Mirrors QwenLoginModal (ours, NOT verbatim).
 *
...
import { ClineLoginContent } from "../../features/agents/components/cline-login-content"
import { useClineLoginFlow } from "../../features/agents/hooks/use-cline-login-flow"
import { clineLoginModalOpenAtom } from "../../lib/atoms"
import { AlertDialog, AlertDialogCancel, AlertDialogContent } from "../ui/alert-dialog"

export function ClineLoginModal() {
  const [open, setOpen] = useAtom(clineLoginModalOpenAtom)
  ...
  const clearPendingRetryIfNeeded = () => {
    if (
      pendingAuthRetry &&
      pendingAuthRetry.provider === "cline" &&
  ...
  useEffect(() => {
    ...
    if (pendingAuthRetry?.provider === "cline" && !pendingAuthRetry.readyToRetry) {
