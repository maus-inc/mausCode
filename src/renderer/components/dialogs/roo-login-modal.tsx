/**
 * mausCode RooLoginModal shell (auth-retry aware). Mirrors ClineLoginModal (ours, NOT verbatim).
 *
...
import { RooLoginContent } from "../../features/agents/components/roo-login-content"
import { useRooLoginFlow } from "../../features/agents/hooks/use-roo-login-flow"
import { rooLoginModalOpenAtom } from "../../lib/atoms"
import { AlertDialog, AlertDialogCancel, AlertDialogContent } from "../ui/alert-dialog"

export function RooLoginModal() {
  const [open, setOpen] = useAtom(rooLoginModalOpenAtom)
  ...
  const clearPendingRetryIfNeeded = () => {
    if (pendingAuthRetry && pendingAuthRetry.provider === "roo" && !pendingAuthRetry.readyToRetry) {
  ...
  useEffect(() => {
    ...
    if (pendingAuthRetry?.provider === "roo" && !pendingAuthRetry.readyToRetry) {
