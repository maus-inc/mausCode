...
export const VSCODE_TO_CSS_MAP: Record<string, string[]> = {
  ...
  "--input-background": ["input.background", "editorWidget.background", "dropdown.background"],
  ...
  "--destructive-foreground": ["editorError.background", "inputValidation.errorBackground"],

  // Timeline/Content background (used for sidebars, dialogs)
  // sideBar.background has priority for consistent sidebar color
  "--tl-background": ["sideBar.background", "panel.background", "editor.background"],
}
 ...
 */
function getColorFromTheme(colors: Record<string, string>, priorityKeys: string[]): string | null {
...
const PRESERVE_ALPHA_VARS = new Set(["--selection"])
...
export function generateCSSVariables(themeColors: Record<string, string>): Record<string, string> {
  ...
  const backgroundColor =
    themeColors["editor.background"] || themeColors["editorPane.background"] || "#000000"
