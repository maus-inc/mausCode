...
function resolveUnixCursorAgentLaunch(): CursorAgentLaunch {
  // Explicit override (community convention; also dodges generic `agent`
  // name collisions with unrelated tools on PATH).
  const override = process.env.CURSOR_AGENT_BIN?.trim()
  if (override) {
    return { command: override, args: ["acp"] }
  }

  // Prefer the specific `cursor-agent` name; fall back to the generic
  // `agent` alias both are shipped by the install script.
  ...
  throw new Error(
    "Cursor agent CLI not found (tried `cursor-agent` and `agent` on PATH). " +
...
export function resolveCursorAgentCliLaunch(extraArgs: string[]): CursorAgentLaunch {
  ...
  if (acpLaunch.args.length >= 2 && acpLaunch.args[1] === "acp") {
    return {
      ...
      args: [...acpLaunch.args.slice(0, 1), ...extraArgs],
    }
  }
