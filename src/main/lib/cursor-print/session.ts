...
/** A parsed NDJSON event from `agent -p --output-format stream-json`. */
export interface CursorPrintEvent {
  type?: unknown
  session_id?: unknown
...
export interface CursorPrintChunk {
  type: string
  id?: string
  delta?: string
  toolCallId?: string
...
export function runCursorPrintTurn(opts: {
  ...
  const handleEvent = (event: CursorPrintEvent) => {
