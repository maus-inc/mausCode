...
/** A parsed NDJSON event from the grok print stream. */
export interface GrokPrintEvent {
  type?: unknown
  data?: unknown
  toolCallId?: unknown
  toolName?: unknown
...
function usageFromPayload(payload: unknown): GrokUsage | undefined {
  ...
  const usage: GrokUsage = {
    ...
    cacheReadInputTokens: num(record.cache_read_input_tokens),
    cacheCreationInputTokens: num(record.cache_creation_input_tokens),
    reasoningTokens: num(record.reasoning_tokens),
...
export function runGrokPrintTurn(opts: {
  ...
  const handleEvent = (event: GrokPrintEvent) => {
