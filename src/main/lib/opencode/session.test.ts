...
import { createOpencodeSession, type OpencodeSessionChunk } from "./session.ts"
...
it("runs a full turn and maps events to chunks with usage", async () => {
  ...
  try {
    const chunks: OpencodeSessionChunk[] = []
    ...
    assert.ok(firstPrompt, "expected a prompt")
    const sentParts = (firstPrompt.body as { parts: Array<Record<string, unknown>> }).parts
