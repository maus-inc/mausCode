...
export async function withGitLock<T>(
  ...
  // Create a new lock for this operation. The executor runs synchronously,
  // so this is always assigned before the finally below reads it.
...
export async function getRepositoryState(worktreePath: string): Promise<{
  ...
  const _isRebasing = status.current?.includes("(no branch") || false
