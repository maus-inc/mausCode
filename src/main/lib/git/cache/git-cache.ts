...
class LRUCache<T> {
  ...
  private evictIfNeeded(incomingSizeBytes: number): void {
    ...
    if (this.config.maxSizeBytes) {
      while (
        this.currentSizeBytes + incomingSizeBytes > this.config.maxSizeBytes &&
        this.cache.size > 0
      ) {
        this.evictLRU()
      }
...
export function estimateSizeBytes(value: unknown): number {
  ...
  if (Array.isArray(value)) {
    return value.reduce((sum, item) => sum + estimateSizeBytes(item), 64)
  }
  if (typeof value === "object") {
    return Object.entries(value).reduce(
      (sum, [key, val]) => sum + key.length * 2 + estimateSizeBytes(val),
      64,
    )
  }
  return 0
...
class GitCache {
  ...
  setFileContent(worktreePath: string, filePath: string, content: string): void {
    const key = `${worktreePath}:${filePath}`
