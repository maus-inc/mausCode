    ...
    for (const line of lines) {
      ...
      switch (chunk.type) {
        ...
        case "claude_session":
          ...
          console.log(
            `[sandbox-import] Claude session chunk: ${chunk.sessionId.slice(0, 8)}... (${chunk.data.length} chars)`,
          )
          break
        case "error":
...
export async function applySandboxGitState(
  ...
  try {
    ...
    if (exportData.bundle) {
      console.log(
        `[sandbox-import] Step 2: Applying git bundle (${exportData.bundle.length} bytes)...`,
      )
      ...
      try {
        ...
        const verifyResult = await execFileAsync(
          "git",
          ["-C", worktreePath, "bundle", "verify", bundlePath],
          {
            timeout: 30_000,
          },
        )
    ...
    if (exportData.stagedPatch) {
      console.log(
        `[sandbox-import] Step 3: Applying staged patch (${exportData.stagedPatch.length} chars)...`,
      )
      ...
      try {
        ...
        // Apply and stage
        await execFileAsync("git", ["-C", worktreePath, "apply", "--cached", stagedPatchPath], {
          timeout: 60_000,
        })
    ...
    if (exportData.unstagedPatch) {
      console.log(
        `[sandbox-import] Step 4: Applying unstaged patch (${exportData.unstagedPatch.length} chars)...`,
      )
    ...
    console.log(
      `[sandbox-import] Step 5: Writing ${exportData.untrackedFiles.length} untracked files...`,
    )
...
export async function importSandboxToWorktree(
  ...
  try {
    ...
    console.log(
      `[sandbox-import] Export response status: ${response.status} ${response.statusText}`,
    )
