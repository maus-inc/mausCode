...
/** Model-message content parts passed to streamText (mirrors the AI SDK part shapes). */
...
type ModelContentPart = ModelTextPart | ModelFilePart
...
function extractPromptFromStoredMessage(message: StoredChatMessage): string {
...
function getLastSessionId(messages: StoredChatMessage[]): string | undefined {
  for (let i = messages.length - 1; i >= 0; i--) {
    const meta: unknown = messages[i]?.metadata
    if (meta && typeof meta === "object" && "sessionId" in meta) {
      const sessionId: unknown = meta.sessionId
...
function isGeminiPreludeChunk(chunk: UIMessageChunk): boolean {
  return (
...
function buildModelMessageContent(
  ...
  const content: ModelContentPart[] = [{ type: "text", text: prompt }]
          ...
          try {
            ...
            const streamWithModel = async (
              ...
              const uiStream = result.toUIMessageStream({
                originalMessages: messagesForStream as UIMessage[],
              ...
              const preludeChunks: UIMessageChunk[] = []
              const flushPrelude = () => {
              ...
              while (true) {
                ...
                if (value?.type === "error") {
                  const rawText =
                    typeof value.errorText === "string" ? value.errorText : "Stream failed"
