...
export const grokRouter = router({
  ...
  chat: publicProcedure
    ...
    .subscription(({ input }) => {
      return observable<GrokStreamChunk>((emit) => {
        ...
        const safeEmit = (chunk: GrokStreamChunk) => {
          if (!isActive) return
          try {
            emit.next(normalizeCodexStreamChunk(chunk) as GrokStreamChunk)
          } catch {
            isActive = false
          }
        }
        ...
        ;(async () => {
          try {
            ...
            let heldErrorChunk: GrokStreamChunk | null = null
            ...
            const handlePrintChunk = (chunk: GrokStreamChunk) => {
              if (chunk?.type === "error") {
