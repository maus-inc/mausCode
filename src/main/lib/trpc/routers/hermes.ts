...
/**
 * Stream chunks emitted to the renderer: the AI SDK chunk dialect plus
 * provider extras (auth-error) emitted around the model stream.
 */
...
export const hermesRouter = router({
  ...
  chat: publicProcedure
    ...
    .subscription(({ input }) => {
      return observable<HermesStreamChunk>((emit) => {
        ...
        const safeEmit = (chunk: HermesStreamChunk) => {
          if (!isActive) return
          try {
            emit.next(normalizeCodexStreamChunk(chunk) as HermesStreamChunk)
          } catch {
            isActive = false
          }
        }
