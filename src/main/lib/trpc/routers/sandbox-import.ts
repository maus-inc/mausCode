...
import { type ExportClaudeSession, importSandboxToWorktree } from "../../git/sandbox-import"
...
async function writeClaudeSession(
  ...
  const isolatedConfigDir = join(app.getPath("userData"), "claude-sessions", subChatId)
...
export const sandboxImportRouter = router({
  ...
  importSandboxChat: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      console.log(
        `[OPEN-LOCALLY] Starting import: remoteChatId=${input.remoteChatId}, remoteSubChatId=${input.remoteSubChatId || "all"}, sandboxId=${input.sandboxId}`,
      )
      ...
      if (remoteChatData.subChats.length > 0) {
        ...
        const messagesArray: StoredChatMessage[] = targetSubChat.messages || []
        const lastAssistant = [...messagesArray].reverse().find((m) => m.role === "assistant")
        targetSessionId = (lastAssistant?.metadata as Record<string, unknown> | undefined)
          ?.sessionId as string | undefined
        console.log(
          `[OPEN-LOCALLY] Target sessionId from subchat messages: ${targetSessionId || "none"}`,
        )
      }
      ...
      console.log(
        `[OPEN-LOCALLY] Received ${importResult.claudeSessions?.length || 0} Claude session(s) from sandbox`,
      )

      if (!importResult.success) {
        console.warn(`[sandbox-import] Git state import failed: ${importResult.error}`)
      ...
      for (const remoteSubChat of remoteChatData.subChats) {
        const messagesArray: StoredChatMessage[] = remoteSubChat.messages || []

        // Find sessionId from last assistant message BEFORE creating subChat
        const lastAssistant = [...messagesArray].reverse().find((m) => m.role === "assistant")
        const messageSessionId = (lastAssistant?.metadata as Record<string, unknown> | undefined)
          ?.sessionId as string | undefined

        // Check if we have a matching Claude session
        const matchingSession =
          messageSessionId && claudeSessions.length > 0
        ...
        if (matchingSession) {
          try {
            ...
            console.log(
              `[sandbox-import] Wrote Claude session for subChat ${createdSubChat.id} with sessionId ${messageSessionId}`,
            )
          } catch (sessionErr) {
  ...
  listRemoteSandboxChats: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const response = await fetch(`${apiUrl}/api/agents/chats?teamId=${input.teamId}`, {
        method: "GET",
        headers: {
          "X-Desktop-Token": token,
        },
  ...
  cloneFromSandbox: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!chatResponse.ok) {
        console.error(
          `[OPEN-LOCALLY] Failed to fetch chat data: ${chatResponse.status} ${chatResponse.statusText}`,
        )
        throw new Error(`Failed to fetch chat data: ${chatResponse.statusText}`)
      }
      ...
      console.log(
        `[OPEN-LOCALLY] Starting sandbox import with full export, sessionId: ${targetSessionId || "all"}`,
      )
      const importResult = await importSandboxToWorktree(
        input.targetPath,
        apiUrl,
      ...
      if (!importResult.success) {
        console.warn(`[OPEN-LOCALLY] Git state import failed: ${importResult.error}`)
        ...
        if (repoFromMeta) {
          ...
          if (metaOwner && metaRepo) {
            ...
            try {
              ...
              try {
                await execAsync(`git remote set-url origin ${finalRemoteUrl}`, {
      ...
      try {
        ...
        console.warn(
          `[OPEN-LOCALLY] Could not get current branch, using fallback: ${actualBranch}`,
          err,
        )
      }
      ...
      console.log(
        `[OPEN-LOCALLY] Existing project:`,
        existingProject ? { id: existingProject.id, name: existingProject.name } : null,
      )
      ...
      for (const remoteSubChat of remoteChatData.subChats) {
        ...
        console.log(
          `[OPEN-LOCALLY] Importing sub-chat: ${remoteSubChat.name} (mode: ${remoteSubChat.mode}, messages: ${messagesCount})`,
        )
        ...
        if (matchingSession) {
          try {
            await writeClaudeSession(createdSubChat.id, input.targetPath, matchingSession)
            console.log(
              `[OPEN-LOCALLY] Wrote Claude session for subChat ${createdSubChat.id} with sessionId ${messageSessionId}`,
            )
          } catch (sessionErr) {
          ...
          console.log(
            `[OPEN-LOCALLY] No matching Claude session found for sessionId: ${messageSessionId.slice(0, 8)}...`,
          )
        } else {
