...
import { type CursorPrintChunk, runCursorPrintTurn } from "../../cursor-print/session"
...
export const cursorRouter = router({
  ...
  chat: publicProcedure
    ...
    .subscription(({ input }) => {
      return observable<CursorPrintChunk>((emit) => {
        ...
        const safeEmit = (chunk: CursorPrintChunk) => {
          if (!isActive) return
          try {
            emit.next(normalizeCodexStreamChunk(chunk) as CursorPrintChunk)
          } catch {
            isActive = false
          }
        }
        ...
        ;(async () => {
          try {
            ...
            let heldErrorChunk: CursorPrintChunk | null = null
            ...
            const handlePrintChunk = (chunk: CursorPrintChunk) => {
              if (chunk?.type === "error") {
