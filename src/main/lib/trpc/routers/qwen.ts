...
import { listQwenStoredModels } from "../../qwen-print/auth-config"
import { type QwenPrintChunk, runQwenPrintTurn } from "../../qwen-print/session"
...
export const qwenRouter = router({
  ...
  chat: publicProcedure
    ...
    .subscription(({ input }) => {
      return observable<QwenPrintChunk>((emit) => {
        ...
        const safeEmit = (chunk: QwenPrintChunk) => {
          if (!isActive) return
          try {
            emit.next(normalizeCodexStreamChunk(chunk) as QwenPrintChunk)
          } catch {
            isActive = false
          }
        }
        ...
        ;(async () => {
          try {
            ...
            let heldErrorChunk: QwenPrintChunk | null = null
            ...
            const handlePrintChunk = (chunk: QwenPrintChunk) => {
              if (chunk?.type === "error") {
