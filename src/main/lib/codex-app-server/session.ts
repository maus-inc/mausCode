...
/** Stream chunks emitted to the router (AI SDK chunk dialect). */
export interface CodexSessionChunk {
  type: string
  id?: string
  delta?: string
  toolCallId?: string
...
export async function createCodexAppServerSession(opts: {
  ...
  const handleCompletedItem = (item: CompletedThreadItem) => {
    ...
    switch (item.type) {
      ...
      case "fileChange": {
        ...
        changes.forEach((change, index: number) => {
          emitToolTrio(
            `${item.id}-${index}`,
            "Edit",
